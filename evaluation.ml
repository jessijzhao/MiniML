...
let eval_s (exp : expr) (_env : Env.env) : Env.value =
  let binop_int (bi : binop) (a : int) (b : int) : expr =
    match bi with
    | Plus -> Num (a + b)
    | Minus -> Num (a - b)
    | Times -> Num (a * b)
    ...
    | LessThan -> Bool (a < b)
  in
  let binop_bool (bi : binop) (a : bool) (b : bool) : expr =
    match bi with
    | Equals -> Bool (a = b)
    | _ -> raise (EvalError "Binop called on invalid expressions")
  in
  let rec eval (exp : expr) : expr =
    ...
    | Binop (bi, e1, e2) -> (match (eval e1, eval e2) with
    ...
    | Conditional (e1, e2, e3) -> (match eval e1 with
                                   ...
                                   | _ -> raise (EvalError "Expression of type
                                                            bool was expected"))
    | Fun (v, e) -> exp
